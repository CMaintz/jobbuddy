  ...
  template: `
    <div class="min-h-screen flex items-center justify-center">
      <div class="card w-full max-w-md">
        ...
        @if (success) {
          <div class="bg-green-50 border border-green-200 text-green-800 rounded-md p-4 text-sm">
            <strong>Check your inbox!</strong><br>
            We've sent a verification link to <strong>{{ registeredEmail }}</strong>.
            Click it to activate your account, then <a routerLink="/login" class="underline">sign in</a>.
          </div>
        } @else {
          @if (error) {
          ...
          <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-4">
            ...
            <div>
              ...
              @if (form.get('password')?.dirty && form.get('password')?.errors?.['weakPassword']) {
                <p class="text-red-600 text-xs mt-1">{{ form.get('password')?.errors?.['weakPassword'] }}</p>
              }
            </div>
            <div>
              <label class="label">Confirm Password</label>
              <input type="password" formControlName="confirmPassword" class="input" placeholder="Re-enter password" />
              @if (form.dirty && form.errors?.['mismatch']) {
